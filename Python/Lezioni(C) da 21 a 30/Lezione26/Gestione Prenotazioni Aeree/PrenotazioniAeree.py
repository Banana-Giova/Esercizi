...
class Volo(ABC):
    def __init__(self, id_volo:str, 
                 posti:int) -> None:
        self.id_volo = id_volo
        self.posti = posti
        self.prenotazioni = 0
...
class VoloCommerciale(Volo):
    def __init__(self, id_volo: str, posti: int) -> None:
        super().__init__(id_volo, posti)
        self.posti_economica = round(posti*0.7)
        self.posti_business = round(posti*0.2)
        self.posti_prima = round(posti*0.1)
    ...
    def posti_disponibili(self):
        return {
            'Posti Disponibili': self.posti,
            'Posti Economica': self.posti_economica,
            'Posti Business': self.posti_business,
            'Posti Prima': self.posti_prima
        }
    
    def prenota_posto(self, pren_posti:int, classe_servizio:str):
        term_pos:str = "Errore, posti terminati."
        ...
        if classe_servizio.casefold() not in valid_classes:
            print("Errore, la classe richiesta è invalida.")
        else:
            if classe_servizio.casefold() == valid_classes[0]:
                if self.posti_economica < pren_posti:
                    print(term_pos)
                else:
                    self.posti_economica -= pren_posti
                    self.posti -= pren_posti
                    self.prenotazioni_economica += pren_posti

            if classe_servizio.casefold() == valid_classes[1]:
                if self.posti_business < pren_posti:
                    print(term_pos)
                else:
                    self.posti_business -= pren_posti
                    self.posti -= pren_posti
                    self.prenotazioni_business += pren_posti

            if classe_servizio.casefold() == valid_classes[2]:
                if self.posti_prima < pren_posti:
                    print(term_pos)
                else:
                    self.posti_prima -= pren_posti
                    self.posti -= pren_posti
                    self.prenotazioni_prima += pren_posti

        print(f"Posti rimanenti: {self.posti}")



class VoloCharter(Volo):
    def __init__(self, id_volo: str, posti: int,
                 prezzo_volo:float) -> None:
        super().__init__(id_volo, posti)
        self.prezzo_volo = prezzo_volo
        self.booked:bool = False

    def prenota_posto(self):
        if not self.booked:
            self.booked = True
            print(f"il volo charter {self.id_volo} è stato prenotato! Prezzo: {self.prezzo_volo}.")
...
class CompagniaAerea:
    def __init__(self, nome_compagnia:str, 
                 prezzo_biglietto:float) -> None:
    ...
    def aggiungi_volo(self, volo_commerciale:VoloCommerciale):
        if isinstance(volo_commerciale, VoloCommerciale):
    ...
    def mostra_flotta(self):
        print(f"Sono parte della flotta di {self.nome_compagnia} i seguenti voli:")
    ...
    def guadagno(self):
        output:float = 0.0
        for i in self.flotta:
            output += i.posti_economica*self.prezzo_biglietto
            output += (i.posti_business*self.prezzo_biglietto)*2
            output += (i.posti_prima*self.prezzo_biglietto)*3
