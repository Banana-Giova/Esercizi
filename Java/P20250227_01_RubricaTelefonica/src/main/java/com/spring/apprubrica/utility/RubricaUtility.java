...
public class RubricaUtility {
	...
	public static ContattoTelefonico INConDTO_OUTCon_NOID(ContattoTelefonicoDTO dto) {
		return new ContattoTelefonico(RubricaUtility.generateNewContact(), dto.getNome(), dto.getCognome(), dto.getNumero(), dto.getGruppo_appartenenza(), dto.getData_nascita(), dto.isPreferito());
	}
	
	public static ContattoTelefonicoDTO INCon_OUTConDTO(ContattoTelefonico con) {
		return new ContattoTelefonicoDTO(con.getContact_id(), con.getNome(), con.getCognome(), con.getNumero(), con.getGruppo_appartenenza(), con.getData_nascita(), con.isPreferito());
	}
